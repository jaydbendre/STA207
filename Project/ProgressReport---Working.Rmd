...
final_df_summ <- final_df %>% group_by(Country, week) %>% 
	summarise(avg_CFR = mean(CFR), debt_relief = getmode(debt_relief), income_support = getmode(income_support), vaccine_availability = getmode(vaccine_availability)) %>% ungroup()
```
...
cols <- c("debt_relief", "income_support", "vaccine_availability")
...
model1 <- plm(avg_CFR  ~ debt_relief  , data = final_df_summ, index = c("Country","week"), model = "random")
